...
scale = 1e6
set ylabel sprintf("Compressibility difference {/Symbol D}Z_{MD} / 10^{–%.0f}", log10(scale))
set format y "%.1f"
set ytics 1
set yrange [-6:6]
set arrow 1 from 0.274,0 to 0.288,0 nohead ls 3
plot file u 1:( scale*($6/y2($1) - 1) ):( scale*$11/y2($1) ) w yerrorbars

set output "plots/cubic.pdf"
plot file u 1:( scale*($6/y3($1) - 1) ):( scale*$11/y3($1) ) w yerrorbars
unset arrow 1
